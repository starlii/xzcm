...
import com.xzcmapi.util.SpringUtils;
    ...
    public static void invokMethod(ScheduleJob scheduleJob) {
        ...
        try {
            if(StringUtils.isNotEmpty(scheduleJob.getParams())){
                method = clazz.getDeclaredMethod(scheduleJob.getMethodName(), Long.class,Integer.class);
            }else{
        ...
        if (method != null) {
            try {
                ...
                if(StringUtils.isNotBlank(scheduleJob.getParams())){
                    String[] split = scheduleJob.getParams().split(",");
                    method.invoke(object, Long.valueOf(split[0]),Integer.valueOf(split[1]));
                }else{
