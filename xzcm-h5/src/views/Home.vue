    ...
    methods: {
      ...
      async getHomeMessage(activityId) {
        ...
        if (data.mualStatus == true) {
          window.location.replace('/#/EndActivity?activityId=' + this.activityId)
        } else {
      ...
      init(appId, getUrl) {
        ...
        if (this.$cookies.get("activityId")) {
          this.$cookies.remove('activityId')
        }
        //id 不等于空的时候跳转到选手页
        if (this.id != null) {
          let urlNow = encodeURIComponent(getUrl + "/#/weixin?activityId=" + this.activityId + "&id=" + this.id)
          ...
          let urlNow = encodeURIComponent(getUrl + "/#/weixin?activityId=" + this.activityId)
          let scope = 'snsapi_base';    //snsapi_userinfo   //静默授权 用户无感知
          let url = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${appId}&redirect_uri=${urlNow}&response_type=code&scope=${scope}&state=1#wechat_redirect`;
          window.location.replace(url);
        }
