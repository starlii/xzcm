  ...
  <div id="home">
    ...
    <section class="pd-10 bg-1 aui-text-center aui-font-size-12 lh-18 bg-white centerInfo">
      <div class="aui-row">
        ...
        <div class="aui-col-xs-4 bd-r-1">
          <div class="text-white"><i class="iconfont icon-dianzan"/> 累计投票</div>
          <div class="aui-font-size-14 cl-1"><strong v-if="data.votes<0">0</strong><strong v-else>{{ data.votes
            }}</strong></div>
        </div>
        <div class="aui-col-xs-4">
          <div class="text-white"><i class="iconfont icon-zhiboguankanshu"/> 访问量</div>
    ...
    <div class="time">
      <ul>
        <li><i class="iconfont icon-riqi"></i> 投票日期：<span>{{ data.voteStartTime | formatDate("yyyy-MM-dd") }} 到 {{ data.voteEndTime | formatDate("yyyy-MM-dd") }}</span>
        </li>
        <li><i class="iconfont icon-shijian aui-margin-r-5"></i>投票倒计时：
            ...
            <li v-for="(item,index) in popularityPlayers">
              <router-link
                ...
                <div class="over-h">
                  ...
                  <div class="index-votes aui-col-xs-4 aui-text-right pd-r-5 aui-pull-right">
                    <span class="red" v-if="item.votes<0">0 票</span>
                    <span class="red" v-else>{{ item.votes }}票</span>
                  </div>
                </div>
                <div class="pos-r aui-margin-t-5">
                  <div class="cl-666 playerTitle aui-pull-left aui-margin-l-5">{{ item.name }}</div>

                  <span class="index-vote bg-1"><i class="iconfont icon-dianzan"></i> 投票</span>
                </div>
              </router-link>
            </li>
          </ul>
...
<style lang="scss">
  #home {
    ...
    .hot li:first-child .index-voteNum, .hot li:nth-child(2) .index-voteNum, .hot li:nth-child(3) .index-voteNum {
      width: 50%;
    }

    .hot li:first-child .index-voteNum {
      font-size: 0.55rem;
      /* Firefox 3.6+ */
      background: -moz-linear-gradient(left, #b12121, #fff);
      /* Safari 4-5, Chrome 1-9 */
      background: -webkit-gradient(linear, left, from(#b12121), to(#fff));
      /* Safari 5.1+, Chrome 10+ */
      background: -webkit-linear-gradient(left, #b12121, #fff);
      /* Opera 11.10+ */
      background: -o-linear-gradient(left, #b12121, #fff);
      /* default */
      background: linear-gradient(left, #b12121, #fff);
      background-color: #1c82d4;
    }

    .hot li:nth-child(2) .index-voteNum {
      font-size: 0.55rem;
      /* Firefox 3.6+ */
      background: -moz-linear-gradient(left, #4453a6, #fff);
      /* Safari 4-5, Chrome 1-9 */
      background: -webkit-gradient(linear, left, from(#4453a6), to(#fff));
      /* Safari 5.1+, Chrome 10+ */
      background: -webkit-linear-gradient(left, #4453a6, #fff);
      /* Opera 11.10+ */
      background: -o-linear-gradient(left, #4453a6, #fff);
      /* default */
      background: linear-gradient(left, #4453a6, #fff);
      background-color: #1c82d4;
    }

    .hot li:nth-child(3) .index-voteNum {
      font-size: 0.55rem;
      /* Firefox 3.6+ */
      background: -moz-linear-gradient(left, #308f3e, #fff);
      /* Safari 4-5, Chrome 1-9 */
      background: -webkit-gradient(linear, left, from(#308f3e), to(#fff));
      /* Safari 5.1+, Chrome 10+ */
      background: -webkit-linear-gradient(left, #308f3e, #fff);
      /* Opera 11.10+ */
      background: -o-linear-gradient(left, #308f3e, #fff);
      /* default */
      background: linear-gradient(left, #308f3e, #fff);
      background-color: #1c82d4;
    }
